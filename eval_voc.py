...
def voc_eval(detpath,
    ...
    # go dwon dets and mark TPs and FPs
    nd = len(image_ids)
    tp = np.zeros(nd)
    fp = np.zeros(nd)
    for d in range(nd):
        ...
        if ovmax > ovthresh:
            ...
            fp[d] = 1. 
    # compute precisoin recall
    fp = np.cumsum(fp)
    tp = np.cumsum(tp)
    rec = tp/float(npos)
    # aviod divid by zero in case the first detection matches a difficult
    # ground truth
